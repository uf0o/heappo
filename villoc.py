...
class Marker(Block):
    ...
    def __init__(self, addr, error=False, **kwargs):
        Block.__init__(self,addr, 0x0, tmp=True, error=error, *kwargs)
...
def RtlAllocateHeap(state, ret, heap, flags, size):
	if not ret:
		state.errors.append("Failed to allocate %#x bytes." % size)
	else:
		state.append(Block(ret, size))
		
def RtlFreeHeap(state, ret, heap,flags, ptr):
	if ptr is 0:
		return
...
def RtlReallocateHeap(state, ret, heap, flags, ptr, size):
...
if __name__ == '__main__':
    ...
    noerrors = codecs.getreader('utf8')(args.ltrace, errors='ignore')
